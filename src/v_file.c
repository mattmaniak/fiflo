...
bool v_file__init(V_file* const v_file)
{
    ...
    v_file__get_actual_line(v_file)->txt = malloc(V_FILE__BASIC_MEMBLK);
...
{
    return v_file__get_actual_line(this)->len == 0;
}

bool v_file__is_actual_line_first(const V_file* const this)
{
    return v_file__get_cursor_y(this) == 0;
}
...
{
    return this->mirrored_cursor_x == v_file__get_actual_line(this)->len;
}
